...
class ArcQQSyncAstrbot(Plugin):
    """ARC QQ Sync (AstrBot Version) 主类"""
    ...
    commands = {
        "bindqq": {
            ...
            "usages": ["/bindqq"],
            "aliases": ["qq"],
            "permissions": ["arc_qq_sync_astrbot.command.bindqq"]
        }
    }
    
    # 定义权限
    permissions = {
        "arc_qq_sync_astrbot.command.bindqq": {
    ...
    def on_load(self) -> None:
        self.logger.info(f"{ColorFormat.BLUE}arc_qq_sync_astrbot {ColorFormat.WHITE}正在加载...{ColorFormat.RESET}")
    ...
    def on_enable(self) -> None:
        ...
        try:
            ...
            welcome_msg = f"{ColorFormat.BLUE}欢迎使用ARC QQ Sync (AstrBot Version)，{ColorFormat.YELLOW}作者：DEVILENMO / ARC-Game-Club{ColorFormat.RESET}"
    ...
    def _handle_bindqq_command(self, sender, command, args):
        ...
        try:
            ...
            if not hasattr(sender, 'name') or not hasattr(sender, 'xuid'):
                sender.send_message(f"{ColorFormat.GRAY}[ARC QQ Sync] {ColorFormat.RED}此命令只能由玩家使用！{ColorFormat.RESET}")
                return True
            ...
            if self.data_manager.is_player_bound(player_name, player.xuid):
                ...
                player.send_message(f"{ColorFormat.GRAY}[ARC QQ Sync] {ColorFormat.GREEN}您的QQ绑定状态：{ColorFormat.RESET}")
                player.send_message(f"{ColorFormat.GRAY}[ARC QQ Sync] {ColorFormat.AQUA}已绑定QQ: {player_qq}{ColorFormat.RESET}")
                player.send_message(f"{ColorFormat.GRAY}[ARC QQ Sync] {ColorFormat.YELLOW}如需重新绑定，请联系管理员{ColorFormat.RESET}")
            else:
    ...
    def on_disable(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"{ColorFormat.YELLOW}arc_qq_sync_astrbot 已禁用{ColorFormat.RESET}")
